...
export const DashboardContainer: React.FC = () => {
  ...
  const [careLogs, setCareLogs] = useState<PendingReview[]>([]);
  ...
  const firestoreCareLogsRef = useRef<PendingReview[]>([]);
  ...
  useEffect(() => {
    ...
    let unsubCareLogs: () => void = () => {};
    let unsubLocalCareLogs: () => void = () => {};
    ...
    try {
      ...
      const mergeCareLogs = () => {
        const merged = [...firestoreCareLogsRef.current, ...(getLocalCareLogs() as unknown as PendingReview[])];
        merged.sort(
          (a, b) =>
            new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime(),
        );
      ...
      unsubLocalCareLogs = subscribeLocalCareLogs(() => mergeCareLogs());
      mergeCareLogs();
      ...
      unsubCareLogs = onSnapshot(
        collection(db, "dailyCareNotes"),
        (snapshot) => {
          const data: PendingReview[] = [];
          snapshot.forEach((doc) => {
            // Include a timestamp if available so they sort properly, otherwise fallback to local time
            let timestamp = new Date().toISOString();
            if (doc.data().generatedAt) {
               timestamp = doc.data().generatedAt.toDate ? doc.data().generatedAt.toDate().toISOString() : timestamp;
            } else if (doc.data().timestamp) {
               timestamp = doc.data().timestamp;
            }
            // Map 'content' into an aiResult shape if needed for timeline rendering
            const raw = doc.data();
            let aiResult = raw.aiResult;
            if (!aiResult && raw.content) {
                aiResult = {
                    observationType: "care_note",
                    observation: raw.content,
                    potentialRiskFlag: "None"
                };
            }
            data.push({ id: doc.id, ...raw, timestamp, aiResult } as PendingReview);
          });
          firestoreCareLogsRef.current = data;
          mergeCareLogs();
        },
        (error) => {
          console.warn("Error fetching care logs: ", error);
          firestoreCareLogsRef.current = [];
          mergeCareLogs();
        },
      );
    } catch (e) {
        ...
        {currentScreen === "profile" &&
          ...
          residents.find((r) => r.id === selectedResidentId) && (
            <ResidentProfile
              ...
              timelineEvents={[...pendingReviews, ...careLogs].filter(r => r.residentId === selectedResidentId).sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())}
